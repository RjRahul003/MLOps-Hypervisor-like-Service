...
class Scheduler:
    ...
    while self.get_queue_length() > 0:
      ...
      with db.session.no_autoflush:
        ...
        cluster.available_gpu >= deployment.required_gpu):
          # Allocate resources
          print(f"Allocating resources for deployment {deployment_id} on cluster
          ...
          print(f"Not enough resources for deployment {deployment_id} on cluster
          
          {cluster.id}. Requeueing...")
  ...
  def requeue_deployment_by_priority(self, deployment_id):
    ...
    {[self.queue.lindex(self.queue_name, i).decode() for i in
    range(self.queue.llen(self.queue_name))]}")
